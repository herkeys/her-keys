...
const LATER_FEATURES = [
  ...
  },
  {
    ...
    owned: [
      ...
      /^src\/notifications\//,
      /^src\/platform\/(deviceLocation|externalIntelligenceClient|localNotifications|supabaseSessionClient)\.ts$/,
      /^src\/store\/LocalNotificationProvider\.tsx$/,
      /^supabase\/blueprints\//,
      /^supabase\/functions\//,
      /^supabase\/migrations\/20260924183000_env_function_alignment\.sql$/,
      /^tests\/weather\//,
      /^tests\/(backendEnvironment|externalIntelligenceArchitecture|localNotificationArchitecture|localNotificationOwnership|localReminderPlan|sessionContinuityTransport|supabaseSessionClient)\.test\.mjs$/,
    ],
    migrations: ['supabase/migrations/20260924183000_env_function_alignment.sql'],
    schemas: [],
    rootCollections: [],
    syncKinds: [],
    shared: [
      ['.env.example', 'Prototype refinements: document backend intent and provider IDs without secrets; Weather device-location refinement: the fixed weather anchor variables are retired (Weather is located by the device, with no build-time location)'],
      ...
      ['src/store/accountRuntimeInstance.ts', 'FR01 and external refinement: share the authenticated data client with session continuity and provider adapters'],
      ...
      ['tests/routeAccess.test.mjs', 'FR01: regression coverage for auth-degraded reconnect access'],
