...
| Life (`app/(app)/life/index.tsx` + lists) | **REFINE** | **MIGRATED** (K7): hub + needs-me/other-tasks lists + shared category list components to canonical rungs (display/supporting/cardTitle); hub section hierarchy (quick capture → "Where things stand" status list) verified on-system; native stack header already token-keyed (household category names preserved) |
