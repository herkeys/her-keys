...
|---|---|---|---|---|---|---|---|
| CREATE SYSTEM | `HouseholdSystem` (+ optional `SystemStep[]`, optional `RecurrenceRule`) | **F04 producer** `commands/createSystem.ts` (steps/recurrence via F04/foundation below) | name non-blank ≤120; an *active* category; ≤ step/system caps; idempotent by draft system id | low. **Not removable afterwards** (no delete/archive exists — see DELETE); everything on it stays editable | none (explicit Save is the deliberate act; draft ≠ canonical) | **YES** | types + storage + `store.commit` exist; only the producer is missing (Addendum A) |
| EDIT SYSTEM (name, purpose, area) | `HouseholdSystem.name / description / categoryId` | **F04 producer** `commands/editSystem.ts` (in-place; `scope`, facets, provenance preserved) | system exists; name non-blank; category active or unchanged; base version fresh | low, reversible (edit again) | none | **YES** | cloud grants UPDATE on `name, description, category_id`; local rows are mutable |
| ADD STEP | `SystemStep` | **F04 producer** `commands/steps.ts` (sparse positions, see ledger §7) | system exists; title non-blank ≤200; effort 0–1440 or unknown; position ≤ 999 | low, reversible only by editing (no remove) | none | **YES** | type + storage exist; `addSystemStep` has no caller and is dense-only |
| EDIT STEP (title, minutes) | `SystemStep.title / effortMinutes` | **F04 producer** `commands/steps.ts` (bumps `updatedAt`; id/position/provenance kept) | step exists; unknown minutes stay `null`, never `0` | low, reversible | none | **YES** | cloud grants UPDATE on `title, effort_minutes` |
...
| REORDER STEP | `SystemStep.position` | **F04 producer** `commands/steps.ts` `moveStep` (minimal-move: one row into a free slot; renumber only when no slot) | ≥2 steps; unique `(systemId, position)` preserved; no dependency implied | low, reversible | none | **YES** | order is meaningful (`stepsInOrder`); cloud grants UPDATE on `position`. Sync caveat MP-04 |
