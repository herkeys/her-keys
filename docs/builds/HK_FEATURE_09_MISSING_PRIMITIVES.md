...
| `HK-INT-MONEY-BOUNDARY-01` | F09 added a genuine, documented shared-file change (`paymentMechanism` facet on `commitment.ts`/`tasks.ts`, M2a) | `tests/meals/boundary.test.mjs` (F08's own protected-file/sibling-ancestry exit gate, baselined at `38ab7f14`) now fails two ways once a Wave 3 branch exists — confirmed to reproduce on unmodified `Her-Keys-W2I` from branch existence alone, independent of any F09 code. Not fixed here (out of F09's scope to edit F08's committed tooling). Integration should retire or rescope this test's temporal assumption before F10+. |
