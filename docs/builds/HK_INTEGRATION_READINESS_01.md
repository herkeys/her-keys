...
| IR8 | `56f1ec7` | ledger, scenario map, compatibility results, gates, completion report (documentation only) |
| IR9 | `50aeb0f` | **OD-A (owner decision): an undecided reading syncs as structured state, never with a title derived from her words** |
| IR9b | `5a054b5` | the real-database hydration journey expects the one deliberate difference of OD-A |
| IR9c | `9dbe02a` | SEMANTICS §4 (what an undecided reading says to the cloud) and the BACKEND privacy rule (documentation only) |
| IR10 | this commit | ledger updated for the owner's resolution of OD-A: gates, feature tips, verdicts, completion report (documentation only) |
...
One decision was NOT the doctrine's to take and was escalated, then **resolved by the owner** (OD-A, §6.6.2): whether a Talk It Out reading she has not yet decided may leave the device with a title derived from her words. The owner's answer is implemented as written: it MAY sync as structured state, the title is a neutral label until she accepts, the accepted canonical title may sync, and nothing is held out of sync as a whole.
...
### 6.4 Test-the-test (`scripts-dev/ir01-mutation-check.cjs`, re-runnable; one node process at a time; 35 mutants)
...
IR7c exists. At the final code tip: **35 caught, 0 survived, 0 broken at `9dbe02a`**.
...
| OD-A | M32 **the projection sends a reading's own title before she has accepted it** · M33 a rejected or superseded reading releases its title · M34 the neutral label carries a slice of her title (quasi-verbatim) · M35 a reading that arrived neutral can be accepted as it stands. (Feature 02's own six OD-A mutants are in `scripts-dev/f02-oda-mutation-check.cjs` on its branch, all caught.) |
...
| Regression that fails when the defect returns | `syncComposition` (37), `productionWiring` (7), `changeBridge` (32), `storeObserve` (10), `pullProgress` (9); M1–M14 | `dependencyStanding` (14); M15–M18 | `durationSource` (24); M19–M25, M30, M31 | `systemSubject` (16); M26–M29 |
...
**OD-A (owner decision) is tested at four levels:** `readingTitle` (14: the projection for every state and kind, apply on the other device, `rowMatchesLocal`, the update patch, `canAccept`); `syncComposition` "OD-A" (6: through `composeAccountApp` with a canary scan of every row the cloud holds — pending, corrected, rejected, an offline chain, a second device); the real database (5 checks in `journey-composition.mjs`: the neutral label in PostgreSQL, no give-away word in any household row or any row the client sent, the accepted title arriving in the same update as the decision and allowed by the real `freeze_decided_interpretation` trigger, a second device that cannot accept a neutral reading; plus `F8`/`F8b` in `sync-integration.mjs`); and Feature 02's own reader and pipeline on `validate/hk-ir01-f02` (§6.6.2).
...
| | Audit tip | Validation tip | tsc | Suite: audit tip → final at the OD-A tip | Verdict |
|---|---|---|---|---|---|
| **F01 Today** | `223d632` | `af7de73` | clean | 1022 → **1236 / 1236** | **READY WITH DOCUMENTED DEBT** |
| **F02 Talk It Out** | `7e6c08b` | `6d2b108` | clean | 1079 → **1301 / 1301** | **READY WITH DOCUMENTED DEBT** (OD-A closed) |
| **F03 Calendar + Capacity** | `90dd7e1` | `31d542d` | clean | 1062 → **1290 / 1290** | **READY WITH DOCUMENTED DEBT** |
| **F04 Systems + Routines** | `6b6a221` | `9f006ee` | clean | 958 → **1160 / 1160** | **READY WITH DOCUMENTED DEBT** |
...
* **The first validation needed no production source change** (everything F02 writes already goes through the store, so the central observer transports it; nothing under its folders imports a sync mechanism). **The owner's resolution of OD-A (below) added one small adaptation in four files under `src/features/talk-it-out/capture/`.**
...
* **OD-A — RESOLVED by the owner, REPAIRED and CLOSED.** *Decision:* undecided Talk It Out readings MAY participate in account-backed sync as structured interpretation state; before explicit acceptance/correction no raw utterance and no title copied from or materially derived from it may sync; the durable/cloud presentation uses a neutral non-source-derived label or other existing structured metadata; after explicit acceptance/correction an accepted canonical title may sync if the domain contract supports it; the whole undecided interpretation is not held out of sync. *Repair (foundation, IR9):* `foundationProjection` sends a reading's title as `titleForCloud(reading)` — its own title only when its state is `accepted`, otherwise `To-do to review` / `Event to review` / `Note to review` by kind; everything else about the reading (kind, dates, amount, child, hint, state, open question, chain, provenance) syncs as before. It is decided when the row is SENT (a reading queued pending and accepted before the push carries its accepted title; an offline chain sends only its decided end); a rejected or superseded reading was never approved and stays neutral. A correction does not release a title (the successor is undecided until she accepts it, and the foundation has no "user-corrected" marker), which is the strictly safer reading of "before explicit acceptance/correction". *The other side:* a reading that ARRIVES neutral (the device that heard her keeps her words) cannot be accepted as it stands — `canAccept` says `needs_title`, F02's coordinator returns `needs-title`, the card offers no save and says why, and she names it through the ordinary correction; the accepted title then syncs. *Regression:* Feature 02's own pipeline (five utterances through the real reader — a task with a duration, an event, a long note-only clause, a clarifying reading, a bill with an amount and an account-number tail) leaves no whole utterance, no 12-character run of it, no canary, no derived title, no 12-character run of a title and no word of five letters or more of a title in any cloud row, cloud call, change-log entry, transport request, claim payload, outbound queue, identity block or diagnostic; free-text answers, corrections and a title she typed herself change nothing; acceptance releases only its own canonical title; rejected and dismissed readings stay neutral; a restart changes nothing; a reader that puts her whole wording in a title is contained before acceptance and still caught by the scan after it. On PostgreSQL the same boundary holds (§6.4). *Interpretation stated for the owner:* I read "durable/cloud presentation" as the record that crosses the account boundary; the device that heard her still keeps the reading, and its derived title, in its own household state exactly as before (SEMANTICS §4). If the local record was meant too, that is a further feature-level change.
* **Still open (owner, not gating):** **OD-B** — a duration she states in a free-text correction is recorded `inferred` (it reaches the reading through the reader): under-claims, never over-claims; recording `user` needs per-field provenance on `Interpretation` or a duration field in the Fix-it form. **OD-C** — an event's end has no provenance: an assumed 30-minute end is durably identical to a stated one, and F03 will read F02's events as fact (the same class as HA-010, for events; pinned by a test as a documented gap).
* Integration must carry: the new test files and the rewritten OD-A tests unchanged; the four small OD-A source changes (`coordinator.ts` `needs-title`, `viewModel.ts` `titleNeeded`, `CaptureCards.tsx`, `copy.ts`); shared files to merge deliberately with the other features (`TalkItOutContext.tsx`, `life/_layout.tsx`, `life/index.tsx`); dedupe F02's feature-local unresolved-capture attention against F01 Today's. Pre-existing gaps stay (no content store; `Interpretation` cannot carry a responsible party; no "user corrected" marker; no consequence field; clarification answer text not stored; Discovery answers have no transport).
* Not run (by rule): F02's own journeys against PostgreSQL, any device pass. (The foundation branch does prove the OD-A boundary against PostgreSQL, including the real freeze trigger.)
...
Computed at code HEAD `9dbe02a` (the OD-A tip). This ledger's own commit changes only `docs/` (`git diff --stat 9dbe02a HEAD -- src app supabase tests package.json scripts-dev` is empty), so the code the gates ran against is the code at the final HEAD. The schema did not change with OD-A: the migration, its hashes and the fingerprint are exactly those of the first pass.

| Gate | Result |
|---|---|
| TypeScript (`tsc --noEmit`, `--max-old-space-size=1600`) | **clean** (exit 0, no diagnostics) |
| Full application suite (serial) | **975 tests / 207 suites, 975 pass, 0 fail, 0 skipped** (baseline at `c2e56b9`: 812 / 812) |
| Targeted integration-readiness suite (`tests/hk-ir01`) | **163 tests / 38 suites, 163 pass** (changeBridge 32, dependencyStanding 14, durationSource 24, productionWiring 7, pullProgress 9, readingTitle 14, storeObserve 10, syncComposition 37, systemSubject 16) |
| Backend harness (`node supabase/tests/run.mjs`, real local PostgreSQL + PostgREST; nothing remote) | **800 / 800 checks** (the previous builds' recorded figure was 684 / 684; not re-run on `c2e56b9` in this session) |
| Account-binding application-composition test | `syncComposition` 37 + `productionWiring` 7 — start from `composeAccountApp`, the function the production root calls; **fail (22 failures) when the composition is disconnected** (M1) |
| Real local multi-device sync journey | `journey-composition.mjs`: **33 checks** against real PostgreSQL / PostgREST / RLS / the real claim RPC, including client A → queue → PostgreSQL → client B, restart, account switch, demo isolation, a 450-task household, and the OD-A boundary |
...
| **OD-A regression: no raw or quasi-verbatim title before acceptance** | `readingTitle` (14) · `syncComposition` OD-A (6) · real-database journey (5) + `sync-integration` F8/F8b · F02's own pipeline (9 tests in its OD-A block, the rewritten note test, and the device-B convergence tests) — all pass; 4 foundation + 6 F02 mutants all caught |
...
| Test-the-test | `scripts-dev/ir01-mutation-check.cjs`: **35 caught, 0 survived, 0 broken at `9dbe02a`** |
...
| Secret scan | 0 hits in 7,627 added lines (JWT, Supabase and Stripe-style keys, AWS, private keys, Google keys, generic secret assignments, service-role material, credentialed URLs); no `service_role` in client source |
...
**CLOSED — OD-A (F02).** Resolved by the owner; implemented, regression-tested in-model, through Feature 02's own pipeline and against PostgreSQL, and closed (§6.6.2, SEMANTICS §4).

**Release gate (not an integration gate):** approval to apply `20260921120000_ir01_duration_source_and_claim_v3.sql` to any real environment. It is additive and was proven on a populated database, but **it must be applied before any environment other than the local stack and before any build of the repaired client is released**: the repaired client sends claim payload v3, which the old server refuses. OWNER-GATED; nothing was applied remotely. Feature integration itself (merging and wiring the four features on the local branches) does not need it.

**Not gating** (integration can proceed in parallel):
1. Adoption of an existing cloud household on a new device (D1): the quarantine UX and rules.
2. Every task saved before HA-010 now reads as an estimate ("not confirmed") until she confirms the duration: accept, or design a one-time "confirm your durations" path.
3. Aggregate capacity claims resting on assumed durations: Today's Daily Load ("Your day fits") is unqualified; Calendar's "Room" is qualified in the sentence but not in the tag. Hedge them, withhold them, or leave them.
4. Wording and placement of the "no longer there" review note across F01 / F03 / F04, and F04's `Needs:` lines.
5. Needs Me `resolved` also meaning "promoted to a task" (IR-D7).
6. A resolve/recover flow for `backlog` and evidence (D10).
7. F01: an unanswered-delegation row about a removed, archived or completed target still says NEEDS YOU (pre-existing; a stale request may still need withdrawing).
8. F02 OD-B (a stated duration in a correction is `inferred`) and OD-C (event-end provenance).
9. F04 (low): the two derived scope rules for subject edits (§6.6.4); MP-01/03/04 remain the owner's earlier decisions.
10. OD-A follow-up (only if the owner meant it): whether the LOCAL record of the device that heard her should also present a neutral label. The decision was implemented as a rule about what crosses the account boundary (SEMANTICS §4); this would be a further, feature-level change.
...
6. **Carry each feature's adaptation files** as listed in its compatibility document; the notable shared ones: F01 `OneMoveSection.durationKnowledge` is a **new required field**; F03's `calendarValidation` import allowlist changed deliberately (`isDone` → `standingOf`); F04's evidence rename; F02's new test files and the rewritten OD-A tests unchanged, its four OD-A source changes (`needs-title`), and the three shared files (`TalkItOutContext.tsx`, `life/_layout.tsx`, `life/index.tsx`) merged deliberately.
...
8. **A reading that arrived neutral is not a titled reading (OD-A).** Any feature that lists or accepts interpretations must treat a title equal to `UNDECIDED_READING_TITLE[kind]` as "unnamed": present the label, offer no save, let her name it (`canAccept` says `needs_title`). Nothing may parse the label for meaning, and no feature may send, log or copy a reading's title to the cloud or anywhere else before it is accepted.
9. **Fakes must not ignore what the server ignores.** IR-D11 hid for a whole build because the fake transport ignored `limit` exactly as the real function does; keep fakes faithful to the server's actual behaviour.
...
| F02 COMPATIBILITY | **READY WITH DOCUMENTED DEBT** (OD-A closed) |
| F03 COMPATIBILITY | **READY WITH DOCUMENTED DEBT** |
| F04 COMPATIBILITY | **READY WITH DOCUMENTED DEBT** |
| **HK-INTEGRATION-READINESS-01** | **READY FOR FEATURE INTEGRATION** |

Every condition of the definition is met in code and evidence: HA-001/009/010/011 repaired; the production app composes the existing sync system; the lifecycle starts and stops it; legitimate pre-binding content cannot stay device-only; no open P0/P1/P2 affecting shared truth; no P3 that makes integration validation misleading; the backend harness passes; the schema state is understood; F01–F04 validate independently; no feature-specific sync architecture exists. **The verdict was READY AFTER OWNER DECISION until the owner resolved OD-A; it is now READY FOR FEATURE INTEGRATION.** What still belongs to the owner is not an integration gate: the production-migration approval is a release gate (§6.9), and the rest are product decisions that integration can proceed beside.
...
2. **Exact final HEAD.** The commit that carries this document on `repair/hk-integration-readiness-01` (`git log -1 --format=%H`). Its code is byte-identical to `9dbe02a` (the gates in §6.7 ran there); it adds only documentation. (The first pass of this report closed at `56f1ec7`; the owner's resolution of OD-A followed as IR9–IR10.)
3. **Commits made.** §6.1: IR0–IR10 (18 commits, of which IR8 and IR10 are documentation) on the repair branch; and, on four isolated validation branches, the ten-plus repair cherry-picks and each validator's adaptation commits. Nothing was pushed, merged, rebased, squashed or amended, nothing was staged with `git add -A`, and no branch, worktree or checkout that is not part of this build was modified.
...
9. **Feature compatibility.** F01 `af7de73` / F02 `6d2b108` / F03 `31d542d` / F04 `9f006ee`, each **READY WITH DOCUMENTED DEBT**, each on its own validation branch at the OD-A tip, each with a compatibility document (§6.6). OD-A, which gated F02, is closed.
10. **Test accounting.** App suite 812 → **975 / 975** (+163, all in `tests/hk-ir01`: 163 tests / 38 suites, 163 pass); harness 684 → **800 / 800 checks**; mutation check **35 / 35 caught** (a surviving mutant found in this build, M24, led to IR7c); feature branches at the OD-A tip: F01 1236, F02 1301, F03 1290, F04 1160, all green. Existing tests edited (no assertion weakened): `claimPayload` (version 3; "every child is sent" replaces "only the closure's child" — the v3 contract), `accountRuntime` (version 3), `foundationAcceptance` ×3 and `foundationOps` (explicit `subjectMemberId: null`; `goalProgress` gains `unavailable`), `syncEngine` (constant renamed, range tightened), fixtures `legacyShapes`/`richHousehold`, SQL 72/73 (an "unknown future version" example 3 → 4), and — for OD-A — `foundationRoundtrip` and the real-database hydration check F8, which now expect the one deliberate difference (an undecided reading's title does not cross; F8b proves it really did not).
...
16. **Owner decisions still open.** §6.9 — **OD-A is closed.** The production-migration approval is a release gate, not an integration gate; the rest are product decisions that do not block integration.
17. **Integration requirements.** §6.10.
18. **Final verdicts.** §6.11 — HA-001 / HA-009 / HA-010 / HA-011 **PASS**; BACKEND FOUNDATION and COMMON FOUNDATION **PASS WITH DOCUMENTED DEBT**; F01 / F02 / F03 / F04 **READY WITH DOCUMENTED DEBT**; **HK-INTEGRATION-READINESS-01 = READY FOR FEATURE INTEGRATION.**
