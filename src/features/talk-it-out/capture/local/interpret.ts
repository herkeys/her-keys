...
import { formatClarificationCode, parseClarificationCode, MAX_CLARIFICATION_STEPS } from '../clarificationCodes';
import type {
  AssumptionCode,
  ClarificationInput,
  ClarificationOption,
...
function eventInstants(date: LocalDate, startMinutes: number, durationMinutes: number, timeZone: string) {
...
function tidyTitle(raw: string): { title: string; shortened: boolean } {
  ...
  if (t.length > TITLE_MAX) return { title: `${t.slice(0, TITLE_MAX - 1)}…`, shortened: true };
  return { title: t, shortened: false };
}

/** A note keeps her clause exactly — trimmed of surrounding punctuation only. */
function verbatimTitle(raw: string): { title: string; shortened: boolean } {
  const t = raw.replace(/\s+/g, ' ').trim().replace(/^[\s,;:.\-–—]+|[\s,;:.\-–—]+$/g, '');
  const cased = t.length > 0 ? t[0].toUpperCase() + t.slice(1) : t;
  return cased.length > TITLE_MAX ? { title: `${cased.slice(0, TITLE_MAX - 1)}…`, shortened: true } : { title: cased, shortened: false };
}
...
const sortSteps = (steps: ClarificationStep[]) => steps.sort((a, b) => STEP_ORDER[a.kind] - STEP_ORDER[b.kind]);
...
// ------------------------------------------------ revising (every route ends here) ---

const draftDate = (draft: ProposalDraft, tz: string): LocalDate | null =>
  draft.startsAt !== null ? logicalDateAt(epochMsOf(draft.startsAt), tz) : draft.dueDate;

/**
 * The ONE function that changes what a proposal says. A clarification answer, a structured edit and a
 * natural-language correction all arrive here as a `ProposalPatch`; there is no second code path that
 * can alter a reading. Returns null when the patch cannot produce a valid reading (for example turning a
 * note into an appointment without being told a time) — nothing is guessed to fill the gap.
 */
export function revise(
  ...
  const openSteps: ClarificationStep[] = draft.clarificationCode === null ? [] : [...(parseClarificationCode(draft.clarificationCode) ?? [])];
  const stillOpen = (predicate: (s: ClarificationStep) => boolean) => openSteps.filter((s) => !predicate(s));
  let steps = openSteps;

  let subjectMemberId = draft.subjectMemberId;
  if (patch.subject) {
    subjectMemberId = patch.subject.kind === 'child' ? patch.subject.memberId : null;
    steps = stillOpen((s) => s.kind === 'which_child');
  }
  ...
  // A question that is already open keeps its provisional value; the request is regenerated from the steps.
  const clarification = requestFor(steps, ctx, anchor);
  return {
...
/** The durable code a revised proposal should carry (null when nothing is left to ask). */
