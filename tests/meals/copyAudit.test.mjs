...
async function everythingSheSeesAndHears() {
  ...
  for (const meals of [[], [{ label: 'Today' }], [{ label: 'Sat 3 Oct' }]]) {
    const s = { ...real() };
    const row = deriveLifeStatus({ categories: categoriesInOrder(s), events: [], tasks: [], systems: [], upcomingMeals: meals.map((m) => ({ ...m, categoryId: 'cat-meals' })), openTaskCounts: openTaskCountsByCategory(s) }).find((x) => x.systemRole === 'meals');
    add([row.value]);
  }
