...
const lifeRow = (state) =>
  deriveLifeStatus({ categories: categoriesInOrder(state), events: [], tasks: [], systems: state.systems, upcomingMeals: upcomingMealsOf(state, TODAY), openTaskCounts: openTaskCountsByCategory(state) }).find((s) => s.systemRole === 'meals');
...
describe('[CG] the Life hub row for Meals is truthful', () => {
  ...
  test('[CG] the row never asks for attention, so it never lowers the Life hub count', () => {
    const statuses = deriveLifeStatus({ categories: categoriesInOrder(real()), events: [], tasks: [], systems: [], upcomingMeals: [], openTaskCounts: openTaskCountsByCategory(real()) });
